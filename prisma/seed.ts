...
async function seedTeams() {
  const newTeams: any[] = [];
  const teams = ['Operations', 'Doctors', 'Patients', 'Partners'];

  for (let i = 0; i < teams.length; i++) {
    await createRandomTeam(teams[i]);
  }
  
  // Fill remaining slots with random teams if needed
  const remainingTeams = TEAM_COUNT - teams.length;
  if (remainingTeams > 0) {
  ...
  async function createRandomTeam(name: string) {
    try {
      ...
      console.error('Error creating team:', ex);
    }
  }
}
...
async function seedTeamMembers(users: any[], teams: any[]) {
  ...
  for (const user of users) {
    const count = Math.floor(Math.random() * (teams.length - 1)) + 2;
    ...
    for (let j = 0; j < count; j++) {
      try {
        ...
        console.error('Error creating team member:', ex);
      }
    }
  }
  ...
  try {
    ...
    console.error('Error bulk creating team members:', ex);
  }
}
...
async function seedInvitations(teams: any[], users: any[]) {
  ...
  for (const team of teams) {
    const count = Math.floor(1 * users.length) + 2;
    for (let j = 0; j < count; j++) {
      try {
        ...
        console.error('Error creating invitation:', ex);
      }
    }
  }
